<?php include("includes/header.php") ?>
    <?php
    $query = "SELECT COUNT(*) total FROM articulos WHERE idCategoria = 6";
    ...
    <div class="my-4 grid grid-cols-1 sm:grid-cols-3  xl:grid-cols-5 text-center mx-16 gap-3">
        <?php
            $query = "CALL PA_consulta_articulos_6();";
            $result = mysqli_query($conn, $query); 
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <a href="tm.php?id=<?php echo $row['id']?>" class="px-4 text-white rounded-md font-medium">Ver Especifíc.</a>
                </div>
            </div>
        <?php }?>
    </div>
