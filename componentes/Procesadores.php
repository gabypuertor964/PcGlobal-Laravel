<?php include("includes/header.php") ?>
    <?php
    $query = "SELECT COUNT(*) total FROM articulos WHERE idCategoria = 2";
    $result = mysqli_query($conn, $query); 
    $row = mysqli_fetch_assoc($result); 
    ?>
        <div class="flex mt-5 mb-4 justify-center font-bold text-lg sm:font-normal sm:text-3xl">
        <h1>Procesadores - <?php echo $row['total']?> Resultados <i class="ms-1 bi bi-search text-sm sm:text-xl"></i></h1>
        </div> 
    <div class="my-4 grid grid-cols-1 sm:grid-cols-3  xl:grid-cols-5 text-center mx-16 gap-3">
        <?php
            $query = "CALL PA_consulta_articulos_2();";
            $result = mysqli_query($conn, $query);
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <a href="proc.php?id=<?php echo $row['id']?>" class="px-4 text-white rounded-md font-medium">Ver Especifíc.</a>
                </div>
            </div>
        <?php }?>
    </div>
